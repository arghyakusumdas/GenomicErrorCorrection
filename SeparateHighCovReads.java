/**
 * Separate High Coverage reads based upon the median coverage of its kmers
 * Usage: ./bin hadoop jar SeparateHighCovReads <ipPath> <opPath> <threshold>
 */
...
public class SeparateHighCovReads extends Configured implements Tool{
	//public static AdditionalUtilities autil = new AdditionalUtilities();
	public static Statistics stat = new Statistics(); 
	public static SeparateHighCovReads shcr = new SeparateHighCovReads();
	
	public static int MAX = 99999, MIN = -1;
	public static class SeparateHighCovReadsMap extends Mapper<LongWritable, Text, Text, Text> {
 		public void map(LongWritable key, Text value, Context context) throws IOException {
			...
			catch(Exception e) {System.out.println("Exception in SeparateHighCovReads.map"); e.printStackTrace();}
 		}
	}
	...
	public int run(String[] args) throws Exception {
		...
		Job job = new Job(conf, "SeparateHighCovReads");
		job.setJarByClass(SeparateHighCovReads.class);
		FileInputFormat.setInputPaths(job, new Path(args[0]));
		FileOutputFormat.setOutputPath(job, new Path(args[1]));
		job.setMapperClass(SeparateHighCovReadsMap.class);
	...
	public static void main(String[] args) throws Exception {
		...
		ToolRunner.run(new Configuration(), new SeparateHighCovReads(), args);
	}
}
